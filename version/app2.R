...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("Alluvial plots use participants with complete 4-year follow-up after filtering.")
    ),
    mainPanel(
      width = 9,
      fluidRow(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          plotlyOutput("yearly_patient_type_plot", height = "380px"),
          tableOutput("yearly_patient_type_table"),
          br(),
...
server <- function(input, output, session) {
  ...
  yearly_patient_type_tbl <- reactive({
    patient_years <- filtered_data() %>%
      distinct(study_id, year)

    first_year_tbl <- patient_years %>%
      group_by(study_id) %>%
      summarise(first_year = min(year), .groups = "drop")

    patient_years %>%
      left_join(first_year_tbl, by = "study_id") %>%
      mutate(patient_type = if_else(year == first_year, "New", "Followed-up")) %>%
      count(year, patient_type, name = "n") %>%
      complete(
        year = c("2022", "2023", "2024", "2025"),
        patient_type = c("New", "Followed-up"),
        fill = list(n = 0)
      ) %>%
      arrange(factor(year, levels = c("2022", "2023", "2024", "2025")), patient_type)
  })

  output$yearly_patient_type_plot <- renderPlotly({
    tbl <- yearly_patient_type_tbl()
    ...
    p <- ggplot(
      ...
      aes(
        ...
        y = n,
        fill = patient_type,
        text = paste0("Year: ", year, "<br>Type: ", patient_type, "<br>N: ", comma(n))
      )
    ) +
  ...
  output$yearly_patient_type_table <- renderTable({
    tbl <- yearly_patient_type_tbl() %>%
      tidyr::pivot_wider(names_from = patient_type, values_from = n) %>%
  ...
  trajectory_wide <- reactive({
    ...
    first_years <- patient_years %>%
      group_by(study_id) %>%
      summarise(first_year = min(year), .groups = "drop")

    tidyr::expand_grid(
      study_id = unique(patient_years$study_id),
      year = years_int
    ) %>%
      left_join(first_years, by = "study_id") %>%
      left_join(
        patient_years %>% mutate(present = TRUE),
        by = c("study_id", "year")
      ) %>%
      mutate(
  ...
  output$followup_trajectory_plot <- renderPlot({
    ...
    ggplot(
      ...
      geom_alluvium(aes(fill = `2022`), alpha = 0.8, width = 0.18) +
  ...
  consecutive_followup_tbl <- reactive({
    patient_followup <- filtered_data() %>%
      distinct(study_id, followup_year)

    tibble(
      followup_stage = 1:4,
      label = c("1st year", "2nd year", "3rd year", "4th year"),
      n_patients = sapply(1:4, function(k) {
        patient_followup %>%
          group_by(study_id) %>%
          summarise(has_all = all(1:k %in% followup_year), .groups = "drop") %>%
          summarise(n = sum(has_all)) %>%
          pull(n)
      })
    )
  })
  ...
  output$consecutive_followup_plot <- renderPlot({
    ...
    ggplot(tbl, aes(x = factor(label, levels = label), y = n_patients)) +
      geom_col(fill = "#5D6D7E", width = 0.7) +
      geom_text(aes(label = comma(n_patients)), vjust = -0.3, size = 4.3) +
      expand_limits(y = max(tbl$n_patients, 0) * 1.12 + 1) +
      labs(
        title = "Participants retained through consecutive follow-up years",
        subtitle = "Counted only if the participant has every year from 1 up to that year",
        x = NULL,
        y = "Number of unique participants"
      ) +
      theme_minimal(base_size = 13) +
  ...
  output$g_alluvial <- renderPlot({
    ...
    validate(need(nrow(wide) > 0, "No parci with complete G-stage data across all 4 years after filtering."))
    ...
    ggplot(flow, aes(axis1 = `1st year`, axis2 = `2nd year`, axis3 = `3rd year`, axis4 = `4th year`, y = pct)) +
      ...
      labs(
        ...
        subtitle = paste0("Complete 4-year follow-up parci: ", nrow(wide)),
        x = NULL,
  ...
  output$risk_alluvial <- renderPlot({
    ...
    validate(need(nrow(wide) > 0, "No participants with complete risk data across all 4 years after filtering."))
    ...
    ggplot(flow, aes(axis1 = `1st year`, axis2 = `2nd year`, axis3 = `3rd year`, axis4 = `4th year`, y = pct)) +
      ...
      labs(
        ...
        subtitle = paste0("Complete 4-year follow-up participants: ", nrow(wide)),
        x = NULL,
        y = "Percent of complete cases",
        fill = "1st year risk"
      ) +
  ...
  map_data <- reactive({
    ...
    out <- banphaeo_sf %>%
      ...
      mutate(
        ...
        filtered_share_pct = dplyr::if_else(
          !is.na(total_patients_all) & total_patients_all > 0,
          100 * n_patients / total_patients_all,
          NA_real_
        )
      )
  ...
  output$subdistrict_map <- renderPlot({
    ...
    dat <- dat %>%
      mutate(
        map_value = suppressWarnings(as.numeric(.data[[metric]])),
        ckd_fill = suppressWarnings(as.numeric(ckd_pct)),
        n_metric = dplyr::case_when(
          metric == "n_patients" ~ n_patients,
          metric == "ckd_pct" ~ n_ckd,
          metric == "no_ckd_pct" ~ n_no_ckd,
          metric == "egfr_alone_pct" ~ n_egfr_alone,
          metric == "uacr_alone_pct" ~ n_uacr_alone,
          metric == "both_pct" ~ n_both,
          metric == "green_pct" ~ n_green,
          metric == "yellow_pct" ~ n_yellow,
          metric == "orange_pct" ~ n_orange,
          metric == "red_pct" ~ n_red,
          TRUE ~ n_patients
        ),
        denom_n = dplyr::case_when(
          metric == "n_patients" ~ total_patients_all,
          TRUE ~ n_patients
        ),
        display_pct = dplyr::if_else(
          !is.na(denom_n) & denom_n > 0,
          100 * n_metric / denom_n,
          NA_real_
        ),
        display_pct = suppressWarnings(as.numeric(display_pct)),
        label_text = paste0(
          subdistrict_th,
          "\n",
    ...
    ggplot(dat) +
      geom_sf(aes(fill = ckd_fill), color = "white", linewidth = 0.7) +
      geom_sf_text(aes(label = label_text), size = 3.1, fontface = "bold", lineheight = 0.95, check_overlap = TRUE) +
      scale_fill_gradient(
        low = "#fff7bc",
        high = "#d95f0e",
        limits = c(0, 100),
  ...
  output$map_note <- renderUI({
    if (is.null(banphaeo_sf)) {
      ...
      HTML(paste0(
        "Map fill is fixed to <b>CKD prevalence (%)</b> with a 0-100 scale.",
        " Labels show <b>numerator/denominator (%)</b> for the selected geographic metric.",
        " For <b>Participant count</b>, the denominator is the total subdistrict N.",
        " For other metrics, the denominator is the filtered participant N in that subdistrict.",
        " Current matched rows", year_note, ": <b>",
